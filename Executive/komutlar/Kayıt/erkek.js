...
message.channel.send(`Kısa sürede 5 den fazla kayıt yaptığınız için yetkileriniz çekildi.`)
        ...
        target.user.username.includes(tag) ? erkekRol.push(ekipRol) : erkekRol = erkekRol;
            ...
            await target.roles.remove(unreg).then(async x => {
                ...
                let embed = new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true})).setDescription(`${target} adlı üye başarılı bir şekilde **Erkek** olarak kaydedildi\n\nBu kullanıcının ${isimler.userName.length} adet isim geçmişi görüntülendi \n${isimler.userName.map(x => x).reverse().join("\n")}`);
